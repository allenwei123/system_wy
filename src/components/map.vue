    ...
    methods:{
      ...
      handleClick(e){
        if (!this.disabled) {
          ...
          this.getAddress([e.lnglat.getLng(),e.lnglat.getLat()]).then(data => {
            this.$emit('click',Object.assign(data,{
        ...
        (op = {}) => {
          ...
          AMap.service('AMap.Geocoder',
            () => {//回调函数
              ...
              if (_this.initParam.initAddress) {//传入初始位置则直接定位
                if (_this.initParam.type === 'default') {
                  _this.updateMap(_this.initParam.initAddress);
                }
                if (_this.initParam.type === 'track'){
                  _this.trackMap(_this.initParam.initAddress);
                }
              }
            });
