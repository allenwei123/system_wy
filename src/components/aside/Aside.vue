    ...
    <el-aside class="c-aside" style="background-color: rgb(238, 241, 246)" width="auto">
        ...
        <el-menu width="200px" v-if="asideData.length" :collapse="isCollapse" background-color="#4a5064" text-color="#fff" :default-active="$route.path" active-text-color="#409EFF">
            <el-menu-item index="0"
                          @click="alink({link:'/home'})">
              <i class="el-icon-menu"></i>
              <span slot="title">工作台</span>
            </el-menu-item>
            <el-submenu
              v-for="(itemNemu,index) in asideData"
              :index="itemNemu.id"
              v-bind:key="index">
              <template slot="title">
                <i class="iconfont " :class="itemNemu.icon">&nbsp;</i>
                <span slot="title" class="c-aside-title">{{ itemNemu.name }}</span>
              </template>
              <el-menu-item v-for="(itemGroup) in itemNemu.group"
                            v-bind:key="itemGroup.key"
                            v-if="itemGroup.show == '1'"
                            :index="itemGroup.link"
                            @click="alink(itemGroup)">
                {{ itemGroup.name  }}
              </el-menu-item>
            </el-submenu>
        </el-menu>
