<template>
	<div class="workergroundwarp" @click.self="hidden" v-if="hiddenType">
		<div class="workerin">
			...
			<div class="leadername" v-for="(item,index) in member">
				<p class="leader_title">{{item.groupName}}</p>
				<ul class="ground_all">
					<li v-for="(items,indexs) in item.groupMembers"><span>{{items.userName}}</span>&nbsp;&nbsp;<span>{{items.mobile}}</span></li>	
				</ul>
			</div>	
...
<script >
	...
	export default{
		...
		watch:{
			workGroupId(newVal,oldVal){
				
				if(newVal){
					this.$xttp.get(projectHttpUrl.classGrounddetail+this.id+'/detail').then(res=>{
						if(res.errorCode==0){
							this.from.groupHeadName = res.data.teamName;

							Object.assign(this.member,res.data.groupList)
						}else{
		...
		methods:{
			//获取班组信息
			getClassList(){
				this.$xttp.get(projectHttpUrl.classGrounddetail+this.workGroupId+'/detail').then(res=>{
					if(res.errorCode==0){
						console.log(res)

						this.form.groupHeadName = res.data.teamHeadName ;
						this.groundName=res.data.teamName
						Object.assign(this.member,res.data.groupList)
			...
			hidden(){
		     ...
		     console.log(this.options[0].id,this.classValue)
		     	for(let x = 0 ; x <this.options.length;x++){
		     		console.log(x)
		     		if(this.options[x].id==this.classValue){
		     			try{
		     			
		     			// Object.assign(this.member,this.options[x])
		     			console.log()
		     			this.form.teamId=this.options[x].id
		     			this.member=this.options[x].groupList
