			...
			<div class="up">
				<div class="form">
					<ul class="form_message">
						...
						<li>
							<span>保养模块：</span>
							<p class="li_pM"><i v-for="(item,index) in data.moduleList " :key="index">{{item}}</i></p>
						</li>

						<li class="spiceli">
							<span>维保人员：</span>
							<div><p v-for="(item,index) in data.groupMembers" :key="index">{{item.userName}}</p></div>
						</li>
						<li>
							<span>截止日期：</span>
						...
						<li>
							<span>执行日期：</span>
							<p class="li_pM"><span class="datashow">{{new Date(data.deadline).toLocaleDateString().replace(/\//g,'-')}}</span> <el-button size='mini' type="danger" disabled>{{data.planStartTime>data.deadline?"提前":data.planStartTime==data.deadline?"执行日":'逾期'}}</el-button></p>
						</li>
				    		...
				    		<div class="select_div" >
				    			 <el-table :data="data.attendanceList">
				    			 	<el-table-column label="状态" :show-overflow-tooltip="true" align="center" width='80px'>
				    			 	  <template slot-scope="scope">{{ scope.row.attendanceStatus}}</template>
				    			 	</el-table-column>
				    	...
				    	<div class="shosemessage">
				    		<div class="select_div">
				    			<ul class="liftMessage_list">
									...
									<li><span>电梯类型：</span><p>{{data.equipType==1?'直梯':data.equipType==2?'扶梯 ':data.equipType==3?'自动人行道':''}}</p></li>
									<li><span>电梯名称：</span><p>{{data.elevatorName}}</p></li>
									<li><span>电梯用途：</span><p>{{data.elevatorName}}</p></li>
									<li><span>注册代吗：</span><p>{{data.registerCode}}</p></li>
									<li><span>开门方式：</span><p>{{openType[data.openType]}}</p></li>
									<li><span>品牌：</span><p>{{data.brandName}}</p></li>
									 <li><span>电梯安装日期：</span><p>{{new Date(data.elevator.installationTime).toLocaleDateString().replace(/\//g,'-')}}</p></li>
	...
	export default{
		...
		data(){
			return{
				...
				value4:3.7,//评分
				value5:4,
				data:{elevator:{installationTime:''}},
