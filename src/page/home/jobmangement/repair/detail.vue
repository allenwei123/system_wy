    ...
    <el-tabs v-if="show" v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="作业记录" name="1">
        <el-table
          ...
          <el-table-column
            label="时间" width="180">
            <template slot-scope="scope">{{scope.row.clockIn | time('yyyy-MM-dd HH:mm:ss')}}</template>
          </el-table-column>
          <el-table-column
            label="位置">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-if="scope.$index > 0 ">
                {{[detail.attendanceList[scope.$index - 1].clockIn ,scope.row.clockIn] | dateDiff}}
              </span>
            </template>
          </el-table-column>
        </el-table>
