<template>
  <el-main>
    <my-direct @click="handleDirectClick" v-if='isProjectChange'></my-direct>
    ...
    <div v-show="!detailType" class="page-list">
      <div class="c-search">
        <el-form :inline="true" :model="form" class="demo-form-inline" >
         <!--  <el-form-item>
          ...
          <el-form-item v-show="isProjectChange">
          <el-select
         ...
         <!--  <el-form-item>
          ...
          <el-form-item> -->
           ...
           <el-form-item>
            <el-select
              ...
              reserve-keyword
              placeholder="维保班组"
              :remote-method="classSearchAsync"
              :loading="ClasssselectLoading" @change='classChange()'>
              <el-option
                v-for="item in classGroundp"
                :key="item.id"
                :label="item.teamName"
          ...
          <el-form-item>
            <el-select
              ...
              reserve-keyword
              placeholder="电梯名称"
              :remote-method="liftSearchAsync"
              :loading="liftselectLoading" @change='liftChange()'>
              <el-option
                v-for="item in liftName"
                :key="item.id"
                :label="item.name"
         ...
         <el-form-item>
            <el-select
              ...
              reserve-keyword
              placeholder="注册码"
              :remote-method="regitSearchAsync"
              :loading="regitselectLoading" @change='regitChange()'>
              <el-option
                v-for="item in regitList"
                :key="item.id"
          ...
          <el-form-item>
            <el-select
              ...
              :loading="elevselectLoading" @change='elevChange()'>
              <el-option
                v-for="item in elevatorList"
                :key="item.id"
                :label="item.elevatorId "
          ...
          <el-form-item>
            ...
            <el-form-item>
          <el-button type="primary" @click="importDialog = true" size="small">导入</el-button>
          </el-form-item>
           <el-form-item>
             <el-button type="primary" @click="exportall" size="small">导出</el-button>
              <!--  <exportExcel :exportType='exportType' :exportUrl="exportUrl" ></exportExcel> -->
           </el-form-item>
            </el-form-item >
          <el-button type="primary" @click="add" size="small" style="margin-top:5px;">新增</el-button>
           </el-form-item>
        </el-form>
        <!-- <div class="c-process"> -->
      ...
      <el-table :data="list" style="width: 100%"
        ...
        <el-table-column label="梯号" :show-overflow-tooltip="true" width="130" align="center">
          <template slot-scope="scope">{{ scope.row.elevatorId }}</template>
        </el-table-column>
        ...
        <el-table-column label="最近保养时间" :show-overflow-tooltip="true" align="center" width="170">
          <template slot-scope="scope">{{scope.row.preMaintenanceDate?new Date(scope.row.preMaintenanceDate).toLocaleDateString().replace(/\//g,'-'):""}}</template>
        </el-table-column>

        <el-table-column label="下次保养时间" :show-overflow-tooltip="true" align="center" width="170">
          <template slot-scope="scope">{{scope.row.nextMaintenanceDate?new Date(scope.row.nextMaintenanceDate).toLocaleDateString().replace(/\//g,'-'):"" }}</template>
        </el-table-column>
...
<script>
  import {certificateHttpUrl,projectHttpUrl,teamHttpUrl} from '../httpUrl';
  ...
  export default {
    ...
    data(){
      return {
        ...
        regitselectLoading:false,
        liftselectLoading:false,
        ClasssselectLoading:false,
        projectOption:[],
        exportType:'remote',
        exportUrl:'/elevator/export',
    ...
    watch:{
       $route(){
        ...
        console.log(this.$router)
          if(this.$route.name=='certificateManage'){
    ...
    methods: {
      ...
      elevSearchAsync(queryString){
         if (queryString.trim() === '') {
          ...
          this.elevselectLoading = true;
           this.$xttp.post(certificateHttpUrl.list,{
              elevatorId:queryString,
              page:1,
              size:10,
            }).then(res => {
              console.log(res)
      ...
      regitChange(){},
      regitSearchAsync(queryString){
          if (queryString.trim() === '') {
            return false;
          }
          this.regitselectLoading = true;
           this.$xttp.post(certificateHttpUrl.list,{
              ...
              console.log(res)
              this.regitList=[];
              this.regitList=res.data.records
              
               this.regitselectLoading = false;
            })
           
            this.regitselectLoading = false;
      },
      liftChange(){},
      liftSearchAsync(queryString){
        console.log(queryString)
           if (queryString.trim() === '') {
            return false;
          }
          this.ClasssselectLoading = true;
           this.$xttp.post(certificateHttpUrl.list,{
              name:queryString,
              page:1,
              size:10,
              ...
              console.log(res)
              this.liftName=[];
              this.liftName=res.data.records
              
               this.liftselectLoading = false;
            })
           
            this.liftselectLoading = false;
      },
      classChange(){},
      classSearchAsync(queryString){
        console.log(queryString)
         if (queryString.trim() === '') {
          return false;
        }
        this.ClasssselectLoading = true;
         this.$xttp.post(teamHttpUrl.list,{
            ...
            console.log(res)
            this.classGroundp=[];
            this.classGroundp=res.data.records
            
             this.ClasssselectLoading = false;
          })
      ...
      //导出
      exportall(){
        // let inputs=`<input name="${houseIds}" value="${this.form.houseIds}">
        //            <input name="${projectName}" value="${this.form.projectName}">
        //            <input name="${name}" value="${this.form.name}">
        //            <input name="${workGroupId}" value="${this.form.workGroupId}">
        //            <input name="${registerCode}" value="${this.form.registerCode}">
        //            <input name="${elevatorId}" value="${this.form.elevatorId}">
        // `;
            exportFun(this.exportUrl,this.$store.getters.token,this.$store.getters.uid, inputs);
      ...
      seeDetail(val){
        ...
        this.$router.push({name:'certificateManageAdd',params:{type:'see',id:val.id}})
      ...
      add(){
        ...
        this.detailType=true;
        this.$router.push({name:'certificateManageAdd',params:{type:'add',id:1}})
        // this.detailType = 'add';
      },
      see(row){
        this.detailType = 'see';
    ...
    created(){
      ...
      if(this.$route.query.id){
         ...
         this.formProject=true;//暂停新增详情功能
