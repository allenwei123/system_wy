        ...
        <el-row>
          ...
          <el-col v-if="disabled" :span="8">
            <el-form-item label="首次服务日期">
              <span>{{detail.startMaintenanceDate | time}}</span>
            </el-form-item>
          </el-col>
        </el-row>
  ...
  export default {
    ...
    computed: {
      ...
      perMonthAmount(){
        ...
        if(!/^[1-9]\d*$|^0\.\d{1,2}$|^[1-9]\d*\.\d{1,2}$/.test(this.form.amount)) return 0;
        ...
        this.$xttp.get(`${httpUrl.detail}/${this.id}/detail`)
          .then(res => {
            if (!res['errorCode']) {
              ...
              //客户名称
              this.userOption = [{
                label: res['data'].propCompanyName,
                value: res['data'].propCompanyId,
              }];
              //维保人员名称
              ...
              //处理下次应收金额
              res['data'].nextPayment = parseFloat(res['data'].nextPayment/10000).toFixed(2);
