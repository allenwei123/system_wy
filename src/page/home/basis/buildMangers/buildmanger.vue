		...
		<div class="warp">
			<!-- 新增详情 -->
			 <transition name='fade'>
				<add ref="addM" @addSuccess='addHidden' @editScuess='editScuess'></add>
			</transition>
			<!-- 楼层对照表 -->
			 <transition name='fade'>
				<floodMap ref="flooldM" @floorMapsave='editScuess'></floodMap>
			</transition>
			<!-- 操作部分开始 -->
			 <el-form :inline="true" :model="form" class="demo-form-inline">
			 	 <el-form-item >
			 	 	<!-- <el-input v-model="form.buildName" placeholder='楼栋名称'></el-input> -->
					 <el-select
         		 ...
         		 <el-form-item >
         		  <el-button type="primary" @click="goSearch" size="small" class="flex">查询</el-button>
         		  </el-form-item>

         		  <el-form-item >
         		  <el-button type="primary" @click="goSearch" size="small" class="flex">导入</el-button>
         		  </el-form-item>
	...
	import {filterParams} from '@/utils/process';
  	...
  	export default{
  		...
  		methods:{
  			buildSearchAsync(query){//获取楼栋名称
  				if(this.communityId){
  				 ...
  				 if (query.trim() === '') {
  				   ...
  				   this.$xttp.post(bulidMangerHttpUrl.list,{
  				      buildName:query,
  			...
  			showDetail(val){
  				console.log(val)
  				 this.$refs.addM.detailId =val.id;
  			...
  			toFloodMap(val){
  				...
  				if(Object.keys(val.floorMap).length==0){
  					this.$refs.flooldM.form.downFloorNum=val.downFloorNum;
  					this.$refs.flooldM.form.upFloorNum=val.upFloorNum;
  					this.$refs.flooldM.isempty=true;
  				}else{
  					Object.assign(this.$refs.flooldM.floor,val.floorMap)
  					this.$refs.flooldM.isempty=false;
  				}
  				this.$refs.flooldM.mapshow=1;
  				this.$refs.flooldM.id = val.id;

  			},
  			add(val){
  				this.$refs.addM.show=true;
  				this.$refs.addM.form.communityId=this.id;
	       ...
	       */
  		 ...
  		 mounted(){
  		 	...
  		 	this.form.communityId=this.$route.params.id;
        this.id=this.form.communityId
