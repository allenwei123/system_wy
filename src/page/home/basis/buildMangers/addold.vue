...
<script>
	...
	export default{
		...
		mounted(){
			console.log(992326599)
			Object.assign(this.initForm,this.form);
			console.log(this.$route.params.typeId)
			this.showOrHidden=this.$route.params.typeId;
			this.id=this.$route.params.id;
			
				if(this.showOrHidden==1){
					console.log(1)
					...
					try{
						this.$nextTick(() => {
							this.$refs.myMap.$emit('mapInit', {
								...
								initAddress:this.form.province+this.form.city+this.form.area+this.form.address//localdetailAddress//`${this.addressSelect.join('')}${this.detailAddress}`
							})
						})
					}catch(e){
						this.$nextTick(() => {
		...
		methods:{
			getBack(){
				...
				this.$router.push({name:'bulidListManger'})
			},
			//编辑
			edit(){
			...
			//选择省份
				addressChange(){
				  ...
				  for(let x = 0 ; x <this.addressData.length;x++){
				    ...
				    if(this.addressData[x].name==this.form.province){
				       ...
				       this.$refs.myMap.$emit('updateMap',this.form.province+this.form.city+this.form.area+this.form.address)
				...
				//地图事件
				mapClick(val){
					if(this.btnChange=="编辑"  && this.$route.params.typeId==2){
						return false;
					}
