        ...
        <div v-if="step1State">
          ...
          <div class="list-wrap">
            <div class="result-block" v-for="item in step1List">
              ...
              <el-button v-if="item.bind === 1" size="small">申请绑定中</el-button>
              <el-button v-else-if="item.bind === 2" size="small">已绑定</el-button>
              <el-button v-else @click="applyRelations(item)" size="small">未绑定</el-button>
            </div>
          </div>
          <div class="noData">
...
//          },
    ...
    methods: {
      ...
      mapClick(val){
        this.addressSelect = [val.province,val.city,val.district];
      ...
      initMap(){
        this.$nextTick(()=>{
          this.$refs.myMap.$emit('mapInit',{
            initAddress: this.type === 'add' ? null : this.step2form.address
          })
        })
      },
    },
