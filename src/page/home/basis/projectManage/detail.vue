<template>
	<div class="detail_message" v-if="true">
		<div class="detail_title">基本信息</div>
		<ul class="detail_list">
			<li>
				<p><span>项目名称:</span>{{showList.projectName}}</p>
...
<script >
	...
	export default{
		...
		mounted(){
			...
			console.log(this.detailId,this.$route.params.id)
			this.$xttp.get(projectHttpUrl.detail+this.detailId+'/detail').then(res=>{
			  if(res.errorCode==0){
			    ...
			    // }
					...
					console.log(localdetailAddress)
					// try{
					// 	this.$nextTick(() => {
							_this.$refs.myMap.$emit('mapInit', {

								initAddress:`${this.addressSelect.join('')}`
							})

							_this.$refs.myMap.$emit("updateMap",localdetailAddress)
					// 	})
					// }catch(e){
					// 	this.$nextTick(() => {
					// 		_this.$refs.myMap.$emit('mapInit', {
